...
>
> **Recommended authoring path:** run `/tech-vision <project-name>` — interactive section-by-section interviewer that forces the load-bearing sections (Current-vs-Target, Anti-scope, Migration path) to actually get filled in instead of left as stubs. Markdown-only output written to `projects/<project-name>/architecture/vision.md`. Re-run quarterly with `r` (refresh) to preserve content across reviews.
